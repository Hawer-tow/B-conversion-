...
public class O2D{
    ...
    public static double dfraction(float number){
        String[] parts2 = String.valueOf(number).split("\\.",2);
        //String beforedecimal = parts2.length > 0 ? parts2[0] : "";
        String afterdecimal = parts2.length > 1 ? parts2[1] : "";

        float dsum = 0;
        if (parts2.length > 1) {

            // float dsum = 0;
            String decimalPart = afterdecimal; // Get digits after the decimal
